    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-black text-neutral-900 tracking-tight flex items-center gap-2">
            <Radio size={22} className="text-[#0ea5e9] animate-pulse" />
        ...
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-sky-50 border border-sky-200 text-[#0ea5e9] text-xs font-bold shadow-sm">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#0ea5e9] opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-[#0ea5e9]"></span>
          </span>
          <span>RADAR LIVE NAY ACTIF</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl p-6 border border-neutral-200/80 shadow-sm flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-sky-50 text-[#0ea5e9] flex items-center justify-center shrink-0 border border-sky-100">
            <ShoppingCart size={24} />
          </div>
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-neutral-500">Paniers Actifs (24h)</p>
        ...
        <div className="bg-[#0A0A0A] text-white rounded-2xl p-6 border border-[#1e1e1e] shadow-xl flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-[#1c1c1c] text-[#0ea5e9] flex items-center justify-center shrink-0 border border-white/10">
            <TrendingUp size={24} />
          </div>
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-[#0ea5e9]">Chiffre d'Affaires Potentiel</p>
            <p className="text-3xl font-black text-white mt-1">{formatMAD(potentialRevenue)}</p>
            <p className="text-xs text-gray-400 mt-0.5">Valeur cumulée des sélections</p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold uppercase tracking-wider text-neutral-700 flex items-center gap-2">
              <Zap size={15} className="text-[#0ea5e9]" /> Flux des Sélections en Direct
            </h3>
            <span className="text-xs text-neutral-500 font-semibold">{liveCarts.length} session(s)</span>
          </div>
              ...
              <div className="space-y-3">
                {topProducts.map((prod, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-2.5 rounded-xl bg-neutral-50 border border-neutral-100">
                    <span className="w-5 text-center font-black text-xs text-[#0ea5e9]">#{idx + 1}</span>
                    <div className="w-10 h-10 rounded-lg bg-white overflow-hidden border border-neutral-200 shrink-0 flex items-center justify-center">
                      {prod.image ? (
