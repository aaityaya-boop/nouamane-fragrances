...
function TeamManagementContent() {
  ...
  // Execute Delete Member (Permanent or Disable)
  const handleDeleteMember = async (permanent: boolean = true) => {
    ...
    try {
      ...
      const url = permanent 
        ? `/api/admin/team/${memberToDelete.id}?permanent=true` 
        : `/api/admin/team/${memberToDelete.id}`;
      ...
      const data = await res.json();
      if (!res.ok) {
        setDeleteError(data.error || 'Erreur lors de la suppression du collaborateur');
        return;
      }

      showToast(
        permanent
          ? `Le compte de "${memberToDelete.name}" a été supprimé définitivement.`
          : `Le compte de "${memberToDelete.name}" a été désactivé.`
      );
      ...
      setDeleteError(err.message || 'Erreur réseau');
    } finally {
  ...
  return (
    <div className="p-6 md:p-10 max-w-[1600px] mx-auto text-neutral-900 space-y-8">
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-neutral-200 rounded-xl max-w-xl w-full p-6 space-y-4 shadow-xl relative my-8 text-neutral-900 animate-in fade-in">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-3.5 text-xs">
              ...
              {/* Danger Zone: Delete Option in Edit Modal */}
              {selectedMember.role !== 'OWNER' && selectedMember.role !== 'CO_OWNER' && selectedMember.id !== currentUser?.id && (
                <div className="pt-2 border-t border-neutral-100">
                  <div className="bg-rose-50/70 border border-rose-200/80 rounded-xl p-3 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-[11px] text-rose-700/90 mt-0.5">
                        Supprimer définitivement le compte ou désactiver les accès de ce collaborateur.
                      </p>
                    </div>
                    <button
                      type="button"
                      ...
                      className="px-3 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-700 text-white text-xs font-medium shrink-0 transition-colors shadow-2xs cursor-pointer flex items-center gap-1.5"
                    >
                      <Trash2 size={13} />
                      <span>Supprimer</span>
                    </button>
      ...
      {/* ======================================================== */}
      {/* MODAL: SUPPRESSION D'UN COLLABORATEUR                    */}
      {/* ======================================================== */}
      {isDeleteModalOpen && memberToDelete && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-neutral-200 rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl relative animate-in fade-in zoom-in-95 text-neutral-900">
            <button
              onClick={() => {
                setIsDeleteModalOpen(false);
                setMemberToDelete(null);
              }}
              className="absolute top-5 right-5 text-neutral-400 hover:text-neutral-700 p-1 rounded-lg transition-colors"
            >
              <X size={16} />
            </button>
            ...
            <div className="flex items-start gap-3.5">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-neutral-500">
                  Cette action aura un impact immédiat sur les accès du collaborateur.
                </p>
              </div>
            </div>
            ...
            {/* Member Card Preview */}
            <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-3.5 flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-neutral-900 text-white flex items-center justify-center text-xs font-bold shrink-0 overflow-hidden">
                {memberToDelete.avatar ? (
                  ...
                  <span>{memberToDelete.name.slice(0, 2).toUpperCase()}</span>
                )}
              </div>
            ...
            <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-3.5 space-y-1.5 text-xs text-amber-900">
              <div className="font-semibold flex items-center gap-1.5 text-amber-800">
                ...
                <span>Conséquences de la suppression :</span>
              </div>
              <ul className="list-disc list-inside space-y-1 text-[11px] text-amber-800/90 pl-1 leading-relaxed">
                <li>Toutes les sessions actives seront révoquées sur-le-champ.</li>
                <li>Le collaborateur ne pourra plus se connecter au tableau de bord.</li>
                <li>Les historiques d&apos;activité et missions existantes seront conservés dans les audits.</li>
              </ul>
            </div>
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-2.5">
              <button
                ...
                className="w-full sm:w-auto px-3.5 py-2 rounded-xl border border-neutral-300 hover:bg-neutral-50 text-neutral-700 text-xs font-medium transition-colors text-center order-3 sm:order-1 cursor-pointer"
              >
                Annuler
              </button>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto justify-end order-1 sm:order-2">
                <button
                  ...
                  className="w-full sm:w-auto px-3 py-2 rounded-xl bg-neutral-100 hover:bg-neutral-200 text-neutral-800 text-xs font-medium transition-colors disabled:opacity-50 text-center cursor-pointer"
                >
                ...
                <button
                  ...
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold shadow-xs disabled:opacity-50 transition-colors cursor-pointer"
                >
                  {deleteLoading ? (
